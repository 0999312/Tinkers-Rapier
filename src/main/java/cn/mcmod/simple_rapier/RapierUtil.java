...
import cn.mcmod.simple_rapier.item.RapierItem;
...
public class RapierUtil {
    ...
    public static void DoStingAttack(ItemStack stack, LivingEntity attacker, LivingEntity target) {
        if (stack.getItem() instanceof RapierItem) {
            RapierItem rapier = (RapierItem) stack.getItem();
    ...
    public static void DoStingAttack(ItemStack stack, float baseDamage, float exDamage, LivingEntity attacker,
        ...
        if (!target.hasItemInSlot(EquipmentSlotType.CHEST)) {
            ...
            if (flag5) {
                ...
                target.level.playSound(null, target, SoundEvents.PLAYER_HURT_SWEET_BERRY_BUSH, target.getSoundSource(), 1.2F, (target.getRandom().nextFloat() - target.getRandom().nextFloat()) * 0.5F + 1.0F);
            }
        }
